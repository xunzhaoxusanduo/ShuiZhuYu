...
{
    static NSString *ID = @"badgeCell";
    TDBadgedCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        cell = [[TDBadgedCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:ID];
    }
    return cell;
}
...
- (void)setItem:(MJSettingItem *)item
{
    ...
    if ([self.item.subtitle isEqualToString:@"0"] || (self.item.subtitle == nil)) {
        self.badgeString = nil;
    }else {
        self.badgeString = self.item.subtitle;
        self.badgeColor = [UIColor redColor];
        self.badgeRightOffset = 5;
//        self.badge.fontSize = 13;
    }
