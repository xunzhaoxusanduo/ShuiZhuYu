...
@property (nonatomic, strong) LXPlayBtn *playOrPauseBtn;
...
- (void)setVideoUrl:(NSString *)videoUrl {
    ...
    //play from start
    [self playOrPause:self.playOrPauseBtn.playOrPauseBtn];
...
- (void)layoutSubviews {
    ...
    if (!self.isOriginalFrame) {
        ...
        self.playOrPauseBtn.layer.cornerRadius = CGRectGetWidth(self.playOrPauseBtn.frame) / 2;
        self.playOrPauseBtn.clipsToBounds = YES;
...
//        [cell.contentView addSubview:self];
        ...
        LXDubbingCell *cell = [self.bindTableView cellForRowAtIndexPath:self.currentIndexPath];
        [cell.videoView addSubview:self];
        NSLog(@"%@", cell);
    }
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        NSLog(@"---------------------------%@", NSStringFromCGRect(self.playerOriginalFrame));
        self.bottomBar.frame = CGRectMake(0, self.playerOriginalFrame.size.height - bottomBaHeight, self.self.playerOriginalFrame.size.width, bottomBaHeight);
        self.playOrPauseBtn.frame = CGRectMake((self.playerOriginalFrame.size.width - playBtnSideLength) / 2, (self.playerOriginalFrame.size.height - playBtnSideLength) / 2, playBtnSideLength, playBtnSideLength);
        self.playOrPauseBtn.layer.cornerRadius = CGRectGetWidth(self.playOrPauseBtn.frame) / 2;
        self.playOrPauseBtn.clipsToBounds = YES;
...
- (void)finishChange {
    ...
    if (self.slider.middleValue) {
        [self.player seekToTime:currentCMTime completionHandler:^(BOOL finished) {
            [self.player play];
            self.playOrPauseBtn.playOrPauseBtn.selected = YES;
        }];
    }
}
...
//            NSLog(@"current --- %f", current );
            ...
            if (weakSelf.slider.value == 1.0f) {      //complete block
                if (weakSelf.completedPlayingBlock) {
                    ...
                    weakSelf.playOrPauseBtn.playOrPauseBtn.selected = NO;
...
//            self.activityIndicatorView.center = self.center;
            ...
            if (self.playOrPauseBtn.playOrPauseBtn.selected) {
                [self.player play];
            }
        }
    }
...
- (void)returnToOriginView {
    ...
    NSLog(@"---------------%@", NSStringFromCGRect(self.currentPlayCellRect));
...
- (UIView *)bottomBar {
    if (!_bottomBar) {
        ...
        slider.minColor = [UIColor orangeColor];
...
- (LXPlayBtn *)playOrPauseBtn {
    if (!_playOrPauseBtn) {
        _playOrPauseBtn = [[LXPlayBtn alloc] init];
        _playOrPauseBtn.layer.opacity = 0.0f;
        [_playOrPauseBtn.playOrPauseBtn addTarget:self action:@selector(playOrPause:) forControlEvents:UIControlEventTouchDown];
    }
