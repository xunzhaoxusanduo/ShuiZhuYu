...
-(void)ym_setNavView{
    ...
    _ym_navView.backgroundColor = NavBarBackgroundColour;
...
//    [_ym_navView addSubview:lineView];
    ...
    [closeBtn setImage:[UIImage imageNamed:@"global_close"] forState:UIControlStateNormal];
...
-(void)closeBtnClick{
    ...
    if (curCityName == nil) {
        UIAlertController *alertVc = [UIAlertController alertControllerWithTitle:nil message:@"您还没有选择城市" preferredStyle:UIAlertControllerStyleAlert];
...
-(void)ym_setCityNames{
    ...
    if (self.ym_cityNames.count) {
        ...
        _ym_selectCity.textColor = [UIColor whiteColor];
        [_ym_selectCity sizeToFit];
        _ym_selectCity.ym_centerX = _ym_navView.ym_centerX;
...
-(void)ym_setLocationManager{
    ...
    if ([_ym_locationManager respondsToSelector:@selector(requestAlwaysAuthorization)]) {
        [_ym_locationManager requestAlwaysAuthorization];
    }
...
-(void)ym_setSelectCityName:(NSString *)cityName{
    // 保存新选择的城市
    [_ym_userDefaults setObject:cityName forKey:LXCurCityNameKey];
    [_ym_userDefaults synchronize];
    // 发出通知
    [[NSNotificationCenter defaultCenter] postNotificationName:LXSelectNewCityNotification object:nil userInfo:@{LXNewCityNameKey: cityName}];
