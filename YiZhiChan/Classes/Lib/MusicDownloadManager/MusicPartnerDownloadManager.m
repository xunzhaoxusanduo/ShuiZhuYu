...
/**
 *  读取已经下载的任务列表，更新任务的下载状态
 */
- (void)updateTaskState {
    NSArray *taskArray = [self loadDownLoadTask];
    for (int i = 0; i < taskArray.count; i++) {
        NSDictionary *dic = [taskArray objectAtIndex:i];
        if (![[dic objectForKey:@"mpDownloadState"] isEqualToNumber:@(3)]) {
            // 已经下载完毕
            if ([self isCompletion:[dic objectForKey:@"mpDownloadUrlString"]]) {
                [dic setValue:@(3) forKey:@"mpDownloadState"];
            }
        }
    }
    
    NSString *filename = [self filePathWithFileName:mpDownLoadTask];
    [taskArray writeToFile:filename atomically:YES];
}
...
-(void)initUnFinishedTask{
    for (NSDictionary *dic in [self loadUnFinishedTask]) {
        ...
        downLoadTask.mpSessionModel.mpDownloadState = (MPDownloadState)[[dic objectForKey:@"mpDownloadState"] integerValue];
...
#pragma mark - MusicPartnerDownloadTaskDelegate代理方法
...
-(void)saveMPDownLoadTask{
    ...
    for (NSString *string in self.mpDownloadTasks.allKeys) {
        ...
        [mpSession setObject:MPFileName(string)            forKey:@"mpDownLoadPath"];
    ...
    [self.mpDownloadArray writeToFile:filename atomically:YES];
}
...
//        if (task.mpSessionModel.mpDownloadState != MPDownloadStateCompleted) {
//            [task start:task.mpSessionModel.urlString];
//        }
    }
}
